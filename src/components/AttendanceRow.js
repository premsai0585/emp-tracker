        ...
        {atndObj.present && (
            <div className="row atnd-list-row py-3" data-bs-theme="dark">
                ...
                <div id={`id-${date}`} className="collapse col-md-8 row d-md-none mx-4 text-muted">
                    ...
                    <div className='col-12'><span className="fw-bold">Hours:</span> {((moment(atndObj.endTime).diff(moment(atndObj.inTime), 'minutes'))/60).toFixed(2)}</div>
                    <div className={`col-12 ${atndObj.late?'text-danger':''}`}>{atndObj.late?'':'Not '}Late</div>
                </div>
                <div className="d-none d-md-flex col-md-8 row text-center">
                    ...
                    <div className={`col-3 ${atndObj.late?'text-danger':''}`}>{atndObj.late?'':'Not '}Late</div>
                </div>
            </div>
        )}
        {!atndObj.present && (
