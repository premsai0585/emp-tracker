        ...
        {atndObj.present && (
            <div className="row atnd-list-row py-3" data-bs-theme="dark">
                ...
                <div id={`id-${date}`} className="collapse col-md-8 row d-md-none mx-4 text-muted">
                    ...
                    <div className={`col-12 ${atndObj.late?'text-danger':'text-primary'}`}>{atndObj.late?'Late':'On Time'}Late</div>
                </div>
                <div className="d-none d-md-flex col-md-9 row text-center">
                    ...
                    <div className={`col-3 ${atndObj.late?'text-danger':'text-primary'}`}>{atndObj.late?'Late':'On Time'}</div>
                </div>
            </div>
        )}
        {!atndObj.present && (
        <div className="row atnd-list-row py-3 bg-secondary" style={{backgroundColor:''}} data-bs-theme="dark">
            <div className="col-md-3 col-6 text-center">{date}</div>
            <div className="col-md-9 text-center col-6 fw-bold" >
                On leave
            </div>
        </div>
        )}
